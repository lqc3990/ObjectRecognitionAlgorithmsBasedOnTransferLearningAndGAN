...
tgt_str = {'USPS_target','USPS_target','SEMEION_target','SEMEION_target','MINIST_target','MINIST_target' };
 Class= 10;
for i_sam = 1:5%1:length(tgt_str)
     % i_sam=3;
    src = src_str{i_sam};
    tgt = tgt_str{i_sam};
    fprintf(' %s vs %s ', src, tgt); 
  
 for iter=1:5 %why 5 Xs only 5;what's iter
    %load Xs_train 源域
    load(['\data\mnist_picture\Handwritten_digits\' src '.mat']); 
     if strcmp(src,'MINIST_source')
      Xs_train =im2double(Xs);
...
%fin_output = fin_output./repmat(sqrt(sum(fin_output.^2)),[size(fin_output,1) 1]);  %//归一化
for i = 1:3
    %ctrl+R.....ctrl+d
    testimage0=reshape(fin_output(i,:),[16 16]);%%%TEST
   testimage0=testimage0';
    imshow(testimage0,[]);%%%TEST
   imwrite(mat2gray(testimage0),strcat(src,num2str(p+i),'.JPG')); %%%TEST 
   x=Xtc_train';
    testimage1=reshape(x(i,:),[16 16]);%%%TEST
     testimage1=testimage1';
   imshow(testimage1,[]);%%%TEST
  imwrite(mat2gray(testimage1),strcat(tgt,num2str(p+i),'.JPG')); %%%TEST
...
%    end
Xat = Xat./repmat(sqrt(sum(Xat.^2)),[size(Xat,1) 1]);  %//归一化
