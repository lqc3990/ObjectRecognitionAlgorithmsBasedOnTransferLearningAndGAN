...
% NNEVAL evaluates performance of neural network。重点是只调用nntest函数
...
nn.testing = 0;
%calc misclassification rate if softmax看这里！！！！！用softmax归一
if strcmp(nn.output,'softmax')
    [er_train, dummy]               = nntest(nn, train_x, train_y);%重点是只调用了nntest函数，
